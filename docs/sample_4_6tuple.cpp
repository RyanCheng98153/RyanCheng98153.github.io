...
public:
	...
	void popup() {
		...
		if (num) {
			// Note: ==> 因為題目更改，出現2的機率=80%，出現4的機率=20%
...
public:
	...
	virtual float update(const board& b, float u) {
		...
		for (int i = 0; i < iso_last; i++) {
			size_t index = indexof(isomorphic[i], b);
			operator[](index) += u / iso_last;   // 這裡可能要乘上 alpha (learning rate)
			updated_value += operator[](index);
		}
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		...
		for (size_t i = 0; i < patt.size(); i++) {
			int tile = b.at(patt[i]);          // 取格子的值
			index |= (size_t(tile) << (4 * i)); // shift & 填進去
		}
		return index;
	}
...
public:
	...
	float update(const board& b, float u) const {
		...
		// Note: Learning rate for features of 𝑛-tuple network with 𝑚 features: 0.1 ÷ 𝑚
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				// =======>
				// 設定 after state 的估值
				const board s_after = move->after_state();
				float v_expect = 0.0f;
				int empty[10], cnt = 0;
				for (int p=0; p<16; p++) {
					if (s_after.at(p) == 0) {
				...
				if (cnt == 0) {
					v_expect = estimate(s_after);
				} else {
					for (int p=0; p<cnt; p++) {
						board b2 = s_after;
						board b4 = s_after;
						b2.set(empty[p], 1); // 2-tile
						v_expect += 0.8f * estimate(b2); // 2-tile 機率80%
						b4.set(empty[p], 2); // 4-tile
						v_expect += 0.2f * estimate(b4); // 4-tile 機率20%
					}
					v_expect /= cnt;
				}

				move->set_value(move->reward() + v_expect);
				// =======>
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		// 	state st = path.back();
		// 	path.pop_back();
		// 	error = target - estimate(st.before_state());
		// 	target = st.reward() + update(st.before_state(), alpha * error);
		// }

		// Modified TD(0) algorithm
		// V(s) = V(s) + alpha * (r + V(s') - V(s))
		// <=> V(s) = V(s) + alpha * error, where error = r + V(s') - V(s)
		// Note: for the final state, V(s') = 0
		// <=> error = r - V(s)
		// =======>
		// 從後往前更新
		// V(s) = V(s) + alpha * (r + V(s') - V(s))
		...
		while (!path.empty()) {
			state st = path.back();
			path.pop_back();
			float vhat = estimate(st.before_state());
			float target = st.reward() + (path.empty() ? 0 : estimate(path.back().before_state()));
			float error = target - vhat;
			update(st.before_state(), alpha * error);
		}
...
// For unique filename, didn't modify the training process
std::string uniquify_filename(const std::string& path);
...
int main(int argc, const char* argv[]) {
	...
	tdl.save(uniquify_filename("model/model.dat"));
...
std::string uniquify_filename(const std::string& path) {
	...
	for (i = 1; ; i++) {
		std::ifstream in;
		in.open((name + (i == 1 ? "" : ("-" + std::to_string(i))) + ext).c_str());
		if (!in.is_open()) break;
		in.close();
	}
	return name + (dot != std::string::npos ? ("-" + std::to_string(i) + ext) : "");
}